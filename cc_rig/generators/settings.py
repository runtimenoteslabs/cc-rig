...
def _script_budget_reminder(config: ProjectConfig) -> str:
    ...
    cost_section = (
        ...
        "    total_in, total_out, total_cache_create, total_cache_read = 0, 0, 0, 0\n"
        "    try:\n"
        "        with open(path) as f:\n"
        "            for line in f:\n"
        "                line = line.strip()\n"
        ...
        "    except (OSError, IOError):\n"
        "        return None\n"
        "    return total_in, total_out, total_cache_create, total_cache_read\n"
        "\n"
        "path = find_latest_jsonl()\n"
        "if not path:\n"
        ...
        "print(f'Session tokens: {total_tokens:,} (in={t_in:,} out={t_out:,}'\n"
        "      f' cache_create={t_cc:,} cache_read={t_cr:,})')\n"
        "print(f'Est. cost ({label}): ~${cost:.2f}')\n"
        "PYEOF\n"
