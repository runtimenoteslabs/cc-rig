...
def generate_claude_md(
    ...
    *,
    ...
    # ── Attribution (HTML comment: visible in file, stable for cache) ──
    # No version number here: it would break prompt cache on every upgrade.
    # Version lives in .cc-rig-manifest.json and .cc-rig.json instead.
    pack_suffix = f" + {config.process_pack}" if config.process_pack else ""
    sections.append(
        f"<!-- Generated by cc-rig"
        f" | {config.workflow}{pack_suffix} + {config.framework}"
        f" | /cc-rig for recipes + savings -->\n"
    )
    ...
    # ── Section 3.5: Workflow Principles (STATIC, non-quick/speedrun) ──
    if config.workflow not in ("quick", "speedrun"):
...
_OUTPUT_STYLE: dict[str, str] = {
    # Tiers
    "quick": "Be terse. No preamble, no recaps, no trailing summaries. Code and actions only.",
    "standard": "Keep explanations to 1-2 sentences. Lead with the answer, not the reasoning.",
    # rigorous: no style constraint (reasoning visibility valued)
...
def _section_process_skills(config: ProjectConfig) -> str:
    ...
    header = config.process_pack or config.workflow
    lines = [f"## Process Skills ({header})", ""]
    ...
    # Process pack attribution
    if config.process_pack == "gstack" or config.workflow_source == "garrytan/gstack":
        lines.append(
